...
from backend.engine.combat.batch import get_total_batch_runs, resolve_batch_execution_plan
...
DEFAULT_SCENARIO_IDS = ("goblin_screen", "orc_push", "marsh_predators")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--batch-size", type=int, default=DEFAULT_BATCH_SIZE, help="Batch size per scenario.")
    parser.add_argument(
        "--player-behavior",
...
def feature_signatures_for_unit(unit: UnitState) -> tuple[str, tuple[str, ...]]:
    ...
    if class_id == "paladin":
        return "paladin", ("lay on hands", "smite", "divine")
...
def run_replay_smoke(
    ...
    for scenario_id in scenario_ids:
        seed = f"party-validation-{player_preset_id}-{scenario_id}-replay"
...
def run_batch_health(
    ...
    for scenario_id in scenario_ids:
        ...
        total_runs = get_total_batch_runs(batch_size, monster_behavior)
        ...
        start_time = time.perf_counter()
        summary = run_batch(config, force_serial=force_serial, worker_count=worker_count)
        elapsed_seconds = time.perf_counter() - start_time
...
def main() -> None:
    ...
    total_runs = get_total_batch_runs(args.batch_size, args.monster_behavior) * len(scenario_ids)
    execution_plan = resolve_batch_execution_plan(
        get_total_batch_runs(args.batch_size, args.monster_behavior),

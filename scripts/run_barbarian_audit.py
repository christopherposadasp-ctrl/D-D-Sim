...
DEFAULT_REPORT_DIR = REPO_ROOT / "reports" / "barbarian_audit"
DEFAULT_JSON_PATH = DEFAULT_REPORT_DIR / "barbarian_audit_latest.json"
DEFAULT_MARKDOWN_PATH = DEFAULT_REPORT_DIR / "barbarian_audit_latest.md"
RULES_GATE_TARGETS = (
    "tests/rules/test_rules.py",
    "tests/rules/test_ai.py",
    "tests/rules/test_barbarian_audit.py",
    "-k",
    "barbarian",
    "-q",
)
...
class BarbarianAuditConsoleProgress:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Barbarian verification audit.")
    parser.add_argument("--scenario", action="append", dest="scenario_ids", help="Audit only the specified scenario id. Repeat to include more.")
    parser.add_argument(
        "--player-preset",
        action="append",
    ...
    parser.add_argument("--fixed-seed-runs", type=int, default=None, help="Override the number of fixed-seed smart/dumb replay checks per scenario.")
    parser.add_argument("--behavior-batch-size", type=int, default=None, help="Override the serial behavior sanity batch size.")
    parser.add_argument("--health-batch-size", type=int, default=None, help="Override the combined health batch size.")
    ...
    parser.add_argument("--markdown-path", type=Path, default=DEFAULT_MARKDOWN_PATH, help="Write the Markdown report to this path.")
    parser.add_argument("--no-report", action="store_true", help="Skip writing report artifacts under reports/.")
    return parser.parse_args()


def build_config_from_args(args: argparse.Namespace) -> tuple[str, BarbarianAuditConfig]:
    mode = "full" if args.full else "quick"
    config = build_full_barbarian_audit_config() if mode == "full" else build_quick_barbarian_audit_config()
...
def main() -> None:
    ...
    selected_scenarios = tuple(args.scenario_ids) if args.scenario_ids else get_barbarian_audit_scenario_ids()
    selected_player_presets = tuple(args.player_preset_ids) if args.player_preset_ids else get_barbarian_audit_player_preset_ids()

    print(
        f"Running Barbarian audit in {mode} mode for "
    ...
    if not args.skip_rules_gate:
        print("Running targeted Barbarian rules gate...")
    ...
    if rules_gate["status"] == "fail":
        payload = {
            ...
            "config": {
                ...
                "baselineReportPath": str(config.baseline_report_path),
            },
        ...
        markdown = "\n".join(
            [
                ...
                *(f"- {line}" for line in rules_gate["stdoutTail"]),
                "",
                "Audit stopped before replay and batch passes because the deterministic Barbarian rules gate failed.",
            ]
        )
    ...
    progress = BarbarianAuditConsoleProgress()
    rows = audit_barbarian_profiles(
        config=config,
    ...
    comparisons = {
        "barbarianSampleTrioVsFighterSampleTrio": build_fighter_trio_comparison(rows, config, progress_callback=progress),
    ...
    markdown = format_barbarian_audit_report(
        rows,
        aggregates,
        rules_gate=rules_gate,
        fighter_comparison=comparisons.get("barbarianSampleTrioVsFighterSampleTrio"),
        baseline_comparison=comparisons.get("martialMixedPartyVsPreBarbarianBaseline"),
    )
